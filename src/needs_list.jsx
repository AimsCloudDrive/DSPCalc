...
import { RequireList } from "./define/RequireList/index.tsx";
    ...
    <>
      <div className="w-fit mt-3 d-flex align-items-center row-gap-1 flex-wrap">
        ...
        <div className="input-group input-group-sm w-fit d-inline-flex me-5">
          ...
          <button className="btn btn-sm btn-outline-danger text-nowrap" onClick={() => set_needs_list({})}>
            清空需求
          </button>
        ...
        {/* {Object.keys(needs_list).length == 0 || <div className="d-inline-flex flex-wrap gap-4 row-gap-0 align-items-center flex-grow-1">{needs_doms}</div>} */}
      </div>
      {/* 选择的需求 */}
      <RequireList list={needs_list} updateList={set_needs_list} />
    </>
  );
}
...
export function NeedsListStorage({ needs_list, set_needs_list }) {
  ...
  let dd_load_list = Object.keys(all_scheme).map((scheme_name) => (
    <li key={scheme_name}>
      <a className="dropdown-item cursor-pointer" onClick={() => load(scheme_name)}>
        {scheme_name}
      </a>
    </li>
  ));
  ...
  let dd_delete_list = Object.keys(all_scheme).map((scheme_name) => (
    <li key={scheme_name}>
      <a className="dropdown-item cursor-pointer" onClick={() => delete_(scheme_name)}>
        {scheme_name}
      </a>
    </li>
  ));
  ...
  return (
    <div className="d-flex gap-2 align-items-center">
      ...
      <div className="input-group input-group-sm">
        <button className="btn btn-outline-secondary" type="button" onClick={save}>
          保存
        </button>
