...
import { ContextProvider, GameInfoContext, GameInfoSetterContext, SchemeDataSetterContext, SettingsSetterContext } from "./contexts.jsx";
...
function GameVersion({ needs_list, set_needs_list }) {
  ...
  async function mods_change(modList) {
    if (JSON.stringify(needs_list) !== "{}" && !confirm(`检测到计算器内有产线，确认继续切换mod吗？切换后将清空产线！`)) {
      return; // 用户取消
    }
...
function UserSettings({ show }) {
  ...
  return (
    <div className={`d-flex gap-3 ${class_show}`}>
      <fieldset>
        <legend>
          <small>设置</small>
        </legend>
  ...
  return (
    <>
      ...
      <div className="d-flex column-gap-4 row-gap-2 flex-wrap">
        ...
        <button className="btn btn-outline-danger btn-sm" onClick={clearData}>
          清空数据缓存
        </button>
