    ...
    if (lstatSync(`./icon/${dir}`).isDirectory()) {
      ...
      return {
        ...
        async buildStart() {
          let { image, coord } = await new Promise((resolve, reject) => {
            Spritesmith.run({ src: glob.sync(`./icon/${dir}/*.png`) }, function (err, result) {
              ...
              let coord_entries = Object.entries(result.coordinates).map(([img, coord]) => [
                path.basename(img, ".png"),
                { ...coord, total_width: width, total_height: height },
              ]);
          ...
          const plugin_options = [imageminPngquant({ quality: [0.1, 0.5], strip: true, speed: 5 }), imageminWebp({ quality: 75 })];
          for (let plugin of plugin_options) {
            await imagemin([output_icon], { destination: "./public/icon", plugins: [plugin] });
          }
          ...
          console.log("icon sprite:", dir, size_before, "->", size_after_png, "MB", "(png)", size_after_webp, "MB", "(webp)");
        },
      };
    } else {
      return [];
